 ...
 *
 * Tools (Phase 1):
 *   propose_tokens(briefPath)   — synth + validate; write a proposal under
 *                                  packages/brand-brief/proposals/<slug>/<id>/
 *
 * Tools planned for Phase 2:
 *   propose_component(spec)
 *   propose_guideline(name, md)
 *
...
import { writeFileEnsureDir, repoRoot, proposalsDir, newProposalId } from "./util/proposalPaths.js";
...
async function main() {
  ...
  server.registerTool(
    ...
    async ({ briefPath }) => {
      ...
      const schemaPath = resolve(root, "packages/brand-brief/schema/brief.schema.json");
      ...
      writeFileEnsureDir(join(proposalRoot, "manifest.json"), JSON.stringify(manifest, null, 2) + "\n");
      writeFileEnsureDir(
  ...
  server.registerTool(
    "propose_component",
    {
      ...
      inputSchema: {
        ...
        name: z.string().describe("Component name (PascalCase). Phase 2: must be 'Button'."),
        variants: z
          .array(z.string())
          .min(1)
          .describe("Ordered list of variants. Must include 'primary'. Allowed: primary, secondary, ghost, danger."),
        sizes: z
          .array(z.string())
          .min(1)
          .describe("Ordered list of sizes. Must include 'md'. Allowed: sm, md, lg."),
      },
    },
    async ({ briefPath, name, variants, sizes }) => {
      ...
      let componentFiles;
      try {
        componentFiles = generateComponent({ briefSlug: slug, name, variants, sizes });
      } catch (e) {
        return {
          isError: true,
          content: [
            { type: "text" as const, text: `propose_component rejected input: ${(e as Error).message}` },
          ],
        };
      }
    ...
    async () => {
      ...
      try {
        ...
        for (const s of slugs) {
          ...
          const ids = await fs.readdir(join(root, s.name), { withFileTypes: true });
          for (const i of ids) {
            if (!i.isDirectory()) continue;
            const manifestPath = join(root, s.name, i.name, "manifest.json");
