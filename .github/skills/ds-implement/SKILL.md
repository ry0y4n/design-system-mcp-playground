...
1. 対象 Brief とガイドライン `name` を確定する。Phase 2 で受理される `name` は `principles` / `voice-and-tone` / `accessibility` / `color-usage` / `do-and-dont` の 5 種。複数生成したい場合は、`name` ごとに 1 回ずつ `propose_guideline` を連続呼び出す。
...
- TSX 本体は決定論的テンプレで生成される（`packages/ds-author-mcp/src/components/button.ts`）。同じくガイドライン Markdown も決定論的テンプレで生成される（`packages/ds-author-mcp/src/guidelines/`）。AI の自由は variants / sizes / guideline name の**選択**のみで、本文を書き換えることはできない。
